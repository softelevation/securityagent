                ...
                <div class="main">
                    <div class="row contacts">
                        <div class="pull-left invoice-to">
                            <div class="text-gray-light">{{__('dashboard.agents.details')}}:</div>
                            <p class="to">{{ucfirst($result->agent->first_name.' '.$result->agent->last_name)}}</p>
                            <div class="address">{{$result->agent->username}}</div>
                            <div class="email"><a href="">{{$result->agent->email}}</a></div>
                        </div>
                        <div class="pull-right">
                            <h2 class="invoice-id">{{__('dashboard.phone_number')}}= {{$result->agent->phone}}</h2>
                        </div>
                        <div class="clearfix"></div>
                    </div>
                </div>
                    ...
                    <table border="0" cellspacing="0" cellpadding="0">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center">{{__('dashboard.payment.status')}}</th>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td class="text-center">{{ ($result->payments->status) ? $result->payments->status : '' }}</td>
                                <td class="text-center">{{ucfirst($result->customer->first_name.' '.$result->customer->last_name)}}</td>
                        ...
                        <tfoot>
                            <tr>
                                <td class="text-right" colspan="5">{{__('dashboard.agents.total_hours_worked')}} {{$result->total_hours}} {{__('dashboard.hours')}}</td>
                                <td class="text-right">{{__('dashboard.grand_total')}} {{$result->amount}} €</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
